...
def init_wiki(project_root: Path, wiki_dir: str, raw_dir: str, upgrade: bool = False, no_raw: bool = False) -> None:
    ...
    raw_targets = [] if no_raw else [(raw, raw_dir), (raw / "assets", f"{raw_dir}/assets")]
    for d, label in raw_targets:
        if not d.exists():
            d.mkdir(parents=True)
...
def main():
    ...
    parser.add_argument("--no-raw", action="store_true", help="Skip creating the raw/ directory (sources live elsewhere, e.g. docs/).")
    ...
    init_wiki(args.project_root, args.wiki_dir, args.raw_dir, upgrade=args.upgrade, no_raw=args.no_raw)
