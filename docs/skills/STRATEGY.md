...
- **Глибина відносних лінків:** `SKILL.md` (корінь скіла) → корінь репо = `../../../`; файли в `reference/`
  на рівень глибше → `../../../../`. До сусіднього скіла з `reference/` = `../../<skill>/...`.
  (Ф1-шаблон помилково використав `../../../` у reference-файлах — виправлено у Ф2; тримати перевірку
  резолву лінків у DoD кожного скіла.)
...
- **Ф2 — решта доменів. ✅ ГОТОВО (2026-07-01).** `bio-qc` (76), `bio-assembly` (analysis+core k-mer/repeat
  ...
  185 битих reference-лінків (Ф1+Ф2) виправлено; freshness зелений.
- **Ф3 — розробник + поліш + pipeline. ✅ ГОТОВО (2026-07-01).**
  `seqeron-dev` (C#-API профіль, символи звірені з src); golden-набір (12 задач) `docs/skills/golden/`;
  CI-guardrail `.github/workflows/skills.yml` (`gen-catalog --check` + `sync --check` + `check-links.py`);
  добудовано **skill-development pipeline** у `.github/agents/` (7 відсутніх агентів + `copilot-instructions.md`);
  **dual-install**: 11 скілів дзеркалено в `.github/skills/` byte-identical (`sync-github-skills.py`, parity в CI).

## 10. Дві системи скілів (як співіснують)

- **Ручний трек (цей документ):** незалежні скіли, згенерований каталог, guardrail-и — те, що вже збудовано.
- **Pipeline-трек (`.github/agents/`):** автоматизований skill-factory (discovery→research→author→adversary→
  verify→eval→coherence) у рантаймі Copilot/VS Code, який *породжує нові* скіли за тими самими конвенціями
  (`copilot-instructions.md`) і встановлює їх у **обидва** дерева. Канонічне джерело скіла — `.claude/skills/`;
  `.github/skills/` — його byte-identical дзеркало (тримається `sync-github-skills.py` + CI parity).
