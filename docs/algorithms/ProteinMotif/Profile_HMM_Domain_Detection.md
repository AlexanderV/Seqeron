...
- `ProteinMotifFinder.FindDomainsByHmm(seq, minBitScore)`: detect SH3/PDZ/WD40 (opt-in).
...
- Exact `hmmsearch` bit score and E-values: MSV/bias/Viterbi/Forward filter pipeline, null2 biased-
  composition correction, and Gumbel/exponential calibration (the `STATS LOCAL` parameters);
  **users should rely on:** HMMER `hmmsearch` for exact bit-score/E-value parity. The bundled
  scorer ranks true vs false positives correctly but its absolute bits differ from `hmmsearch`.
...
| 2 | Absolute bit score ≠ `hmmsearch` | Deviation | Ranking correct; absolute bits/E-value not calibrated | accepted | honest residual; §6.2 |
...
Only three Pfam domains are bundled (SH3, PDZ, WD40); the full Pfam library is not embedded. The
score is a faithful Plan7 log-odds but **not** an exact HMMER `hmmsearch` bit score: the filter
pipeline, null2 correction and statistical calibration are out of scope, so absolute bit scores and
E-values are not reproduced (ranking and detection are). Glocal scoring reports whole-sequence
spans, not per-domain envelopes.
