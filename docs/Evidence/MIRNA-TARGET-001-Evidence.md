...
| Garcia DM et al. (2011) Nat Struct Mol Biol 18:1139 | WebFetch https://pmc.ncbi.nlm.nih.gov/articles/PMC3190056/ (2026-06-26) | Online Methods: *"TA in the human transcriptome was calculated as the number of non-overlapping 3′UTR 8mer, 7mer-m8, and 7mer-A1 sites in the reference mRNAs."* Results: *"each additional CG dinucleotide imparted an additional log10 reduction in TA"* ⇒ TA is log10-scaled. doi:10.1038/nsmb.2115 |
| `TA_SPS_by_seed_region.txt` (TargetScan distribution) | WebFetch https://raw.githubusercontent.com/nsoranzo/targetscan/main/TA_SPS_by_seed_region.txt (2026-06-26) | Header `Seed region \t SPS (8mer and 7mer-m8) \t SPS (7mer-1a and 6mer) \t TA`; one row per 7-nt seed; TA column (col 4) ≈ 1.64–4.37 (e.g. `AAAAAAA … 4.371`) ⇒ log10(site count). |
| `targetscan_70_context_scores.pl` — TA usage | WebFetch https://raw.githubusercontent.com/nsoranzo/targetscan/main/targetscan_70_context_scores.pl (2026-06-26) | `$garcia{$seedRegion}{"TA"} = $TA` (4th column) and `getAgarwalContribution($siteType,"TA_3UTR",$garcia{$seedRegion}{"TA"})` — TA value used **as-is** (already log10) into the min-max scaling branch. |
| Agarwal et al. (2015) eLife Table 1 — TA_3UTR | WebFetch PDF https://rcastoragev2.blob.core.windows.net/.../PMC4532895.pdf (2026-06-26) | TA_3UTR = "Number of sites in all annotated 3′ UTRs (Arvey et al., 2010; Garcia et al., 2011)"; listed among the `context+` features (`min_dist, local_AU, 3P_score, SPS, and TA`) inherited from Garcia 2011. |
| Arvey A et al. (2010) Mol Syst Biol 6:363 | WebSearch + https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2872614/ (2026-06-26) | Original "target abundance dilutes microRNA/siRNA activity": regulation depends on total concentration of available target transcripts (rationale for the abundance feature). doi:10.1038/msb.2010.24 |
...
### TA_3UTR — target-site abundance, now computed from a 3'UTR set (Garcia 2011 / Agarwal 2015)

**Definition (verbatim, retrieved this session).** Garcia et al. (2011) *Nat Struct Mol Biol* 18:1139, Online Methods (WebFetch https://pmc.ncbi.nlm.nih.gov/articles/PMC3190056/, 2026-06-26): *"TA in the human transcriptome was calculated as the **number of non-overlapping 3′UTR 8mer, 7mer-m8, and 7mer-A1 sites in the reference mRNAs**."* Agarwal et al. (2015) eLife Table 1 (WebFetch PDF, 2026-06-26) names the context++ feature **`TA_3UTR` = "Number of sites in all annotated 3′ UTRs (Arvey et al., 2010; Garcia et al., 2011)"**.

**Log10 transformation (verbatim, retrieved this session).** TargetScan stores **one TA value per 7-nt seed region** in `TA_SPS_by_seed_region.txt`. Its header (WebFetch https://raw.githubusercontent.com/nsoranzo/targetscan/main/TA_SPS_by_seed_region.txt, 2026-06-26) is `Seed region \t SPS (8mer and 7mer-m8) \t SPS (7mer-1a and 6mer) \t TA`; the TA column (col 4) ranges **≈ 1.64 – 4.37** (e.g. `AAAAAAA … 4.371`). `targetscan_70_context_scores.pl` reads `$garcia{$seedRegion}{"TA"}` and feeds it **as-is** to `getAgarwalContribution($siteType,"TA_3UTR", …)`; the regression min/max for TA are ≈ 3.07–3.89. A raw count of ~10³–10⁴ matching the 3.07–3.89 scaling window, plus Garcia 2011's statement that *"each additional CG dinucleotide imparted an additional **log10** reduction in TA"*, establish that the stored TA value is **log10(count)**. Therefore:

> **TA = log10(N)**, where **N** = total number of non-overlapping **8mer + 7mer-m8 + 7mer-A1** sites of the seed summed over every 3'UTR in the set.

**Implementation.** `ComputeTa3Utr(MiRna, IEnumerable<3'UTR>)` (with `CountSeedSites3Utr` exposing the raw `N`) counts, in each 3'UTR, every occurrence of the 6mer core (reverse complement of miRNA positions 2-7) that additionally has the position-8 match (7mer-m8 / 8mer) and/or an A opposite miRNA position 1 (7mer-A1 / 8mer) — exactly the three Garcia site types — and returns `log10` of the total (0 when the count is 0; log10(0) is undefined and TargetScan never emits a no-site seed). Site definitions match `FindTargetSites`. The result feeds `ScoreTargetSiteContextPlusPlus` via `ContextPlusPlusInputs.Ta`, which applies the bundled per-site-type Agarwal TA coefficient + min-max scaling. **Hand-derived cross-check** (encoded in CTX-TA-002): for hsa-let-7a-5p (seed `GAGGUAG`, 6mer core `UACCUC`, pos8 base `C`) over the five UTRs `GGCUACCUCAGG` (8mer), `GGCUACCUCGGG` (7mer-m8), `GGGUACCUCAGG` (7mer-A1), `GGGUACCUCGGG` (bare 6mer, excluded), `CUACCUCACUACCUCG` (8mer + 7mer-m8) ⇒ N = 1+1+1+0+2 = 5 ⇒ TA = log10(5) = 0.698970004336019, reproduced exactly by `ComputeTa3Utr`.

### Still caller-supplied (data-blocked) features — SPS / Len_ORF / ORF8m / PCT sigmoid params

These cannot be derived from a 3'UTR set alone: **SPS** is a lookup in the Garcia et al. (2011) `TA_SPS_by_seed_region.txt` table (4096 seed regions; not a closed-form formula), **Len_ORF** needs the transcript ORF length, **ORF8m** needs a precomputed ORF 8mer count, and the **PCT per-family sigmoid parameters** `b0..b3` live in TargetScan's compiled, citation-required tables (Friedman 2009 does not publish them as numbers). The implementation accepts each as an OPTIONAL caller-supplied value and applies the exact `getAgarwalContribution` math (SPS/Len_ORF min-max scaled — Len_ORF after `log10`; ORF8m used raw); when not supplied they stay residual.
