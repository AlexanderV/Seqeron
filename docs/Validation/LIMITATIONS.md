...
|------|----------|-------------|
| PRIMER-TM-001 | Triloop/tetraloop & terminal-mismatch special-loop **bonus tables** for the hairpin loop (the bonus is caller-supplied via `loopBonusDeltaG37`); and the `ntthal` internal-loop / terminal-overhang (`tstack2`) extension of the dimer alignment (the gapless contiguous-WC dimer Tm itself is implemented). Self-dimer / cross-dimer (intermolecular) Tm is now **implemented** (`FindMostStableDimer` / `CalculateDimerMeltingTemperature` / `CalculateSelfDimerMeltingTemperature`). | UNAFold, ViennaRNA, MELTING 5. |
...
| PROTMOTIF-DOMAIN-001 | HMMER's **stochastic-traceback clustering** of *closely-overlapping* multi-domain regions (`region_trace_ensemble`); and Pfam coverage beyond the three bundled domains. | Well-separated multi-domain decomposition (region detection + per-envelope null2-corrected score / i-Evalue, e.g. a 7-blade WD40 β-propeller) is now implemented and matches `hmmsearch` (pyhmmer); only the sampling clusterer for not-well-separated domains is deferred. Any additional Pfam profile is a caller-supplied `.hmm`. |
