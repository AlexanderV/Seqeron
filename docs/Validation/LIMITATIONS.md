...
|------|----------|-------------|
| PRIMER-TM-001 | The `ntthal` internal-loop / terminal-overhang (`tstack2`) extension of the dimer alignment (the gapless contiguous-WC dimer Tm is computed); and the triloop/tetraloop & terminal-mismatch special-loop hairpin **bonus tables** (caller-supplied via `loopBonusDeltaG37`). | UNAFold, ViennaRNA, MELTING 5. |
