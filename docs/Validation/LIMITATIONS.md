...
|------|----------|-------------|
| PRIMER-TM-001 | The optional caller-supplied triloop/tetraloop & terminal-mismatch special-loop **hairpin** bonus tables (via `loopBonusDeltaG37`). The full `ntthal` **dimer** DP — internal mismatches/loops, bulges and terminal overhangs (`tstack2`) — is now implemented (`CalculateDimerThermodynamicsNtthal`) and matches primer3-py 2.3.0 for non-contiguous optima. | UNAFold, ViennaRNA, MELTING 5 (for the hairpin special-loop bonuses only). |
...
| PROTMOTIF-DOMAIN-001 | **Bit-exact RNG parity** of HMMER's stochastic-traceback ensemble (`region_trace_ensemble`); and Pfam coverage beyond the three bundled domains. | The clustering itself is now implemented (opt-in `FindDomains`, default on): closely-overlapping multi-domain regions are split via the 200-sample stochastic-traceback ensemble + single-linkage consensus, and the envelope **count and coordinates** match `hmmsearch` (pyhmmer 0.12.1) **exactly** (scores within ~0.06 bits), with a verbatim port of Easel's fixed-seed LCG. Bit-for-bit identity of every sampled trace would need HMMER's float32 Forward matrix/`FLogNorm`/`FChoose` (this engine uses float64); the 200-sample consensus is robust to this, so only per-sample trace bit parity is unreached. Any additional Pfam profile is a caller-supplied `.hmm`. |
