...
| PROTMOTIF-DOMAIN-001 | Exact HMMER `hmmsearch` bit-score / E-value parity, and Pfam coverage beyond the three bundled (CC0) domains. | The full `hmmsearch` pipeline (MSV/bias filters, null2 biased-composition correction, Gumbel E-value calibration) and the full Pfam library are out of scope. |
