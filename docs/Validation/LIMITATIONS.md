...
| CODON-CAI-001 | Exclusion of single-codon amino acids | Met (ATG) and Trp (TGG) are included with relative adaptiveness w=1.0 rather than excluded per Sharp & Li 1987 / Jansen 2003. Bounded in [0,1]; geometric-mean CAI otherwise exact. |
| CODON-RARE-001 | Rare-codon **cluster / run** detection | Flags rare codons per-codon (relative-fraction threshold, default 0.15); does not detect consecutive rare-codon clusters / ramps. |
