...
| MIRNA-PRECURSOR-001 | The **read-stacking** (small-RNA-seq pileup) signal of miRDeep2 — the read-support component of its probabilistic precursor score. (A bundled **trained** structure/sequence-feature natural-vs-background classifier now ships: `ClassifyPreMiRna`, logistic regression over MFE/AMFE/MFEI/GC/%paired, trained from public-domain miRBase positives vs di-shuffled negatives — Bonnet 2004; held-out AUC = 1.0. NO GPL miRDeep2 code is used.) | miRDeep2 with the caller's small-RNA-seq reads (for the read-stacking signal). |
