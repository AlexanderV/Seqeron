...
| MIRNA-TARGET-001 | The context++ feature `TA_3UTR` (transcriptome-wide site abundance) is caller-supplied. `PCT` (multi-species conservation) is now computed from a caller-supplied alignment + tree (Friedman 2009 branch-length score → published logistic; the per-family sigmoid parameters are caller-supplied because TargetScan's compiled tables are citation-required and Friedman 2009 does not publish them as numbers). `SA` is computed; `SPS` / `Len_ORF` / `ORF8m` are caller-supplied. The score is a partial context++. | A transcriptome for `TA_3UTR`; for `PCT`, supply the multi-species alignment + tree + the published per-family sigmoid parameters. |
