...
Exposes **35 tools** wrapping the `Seqeron.Genomics` library. Every tool has an
explicit JSON input/output schema, a Schema+Binding test, and per-tool docs under
[`docs/mcp/tools/sequence/`](../../../../docs/mcp/tools/sequence) — see the
...
Register it in any MCP client as a stdio server (`command: dotnet`, `args: ["run","--project","Seqeron.Mcp.Sequence"]`). See [`docs/mcp/README.md`](../../../../docs/mcp/README.md).
