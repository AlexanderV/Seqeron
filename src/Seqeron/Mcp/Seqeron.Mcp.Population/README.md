...
Exposes **18 tools** wrapping the `Seqeron.Genomics` library. Every tool has an
explicit JSON input/output schema, a Schema+Binding test, and per-tool docs under
[`docs/mcp/tools/population/`](../../../../docs/mcp/tools/population) — see the
...
Register it in any MCP client as a stdio server (`command: dotnet`, `args: ["run","--project","Seqeron.Mcp.Population"]`). See [`docs/mcp/README.md`](../../../../docs/mcp/README.md).
