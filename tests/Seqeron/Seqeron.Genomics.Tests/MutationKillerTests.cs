...
public class MutationKillerTests
{
    ...
    // ═══════════════════════════════════════════════════════════════════
    // RepeatFinder.cs — FindPalindromes (completely untested)
    // ═══════════════════════════════════════════════════════════════════

    #region RepeatFinder — FindPalindromes (zero coverage)

    [Test]
    public void FindPalindromes_EcoRI_GAATTC_Found()
    {
        // EcoRI recognition site is a palindrome
        var results = RepeatFinder.FindPalindromes(
            new DnaSequence("ATGAATTCGC"), minLength: 6, maxLength: 6).ToList();

        results.Should().ContainSingle();
        results[0].Position.Should().Be(2);
        results[0].Sequence.Should().Be("GAATTC");
        results[0].Length.Should().Be(6);
    }

    [Test]
    public void FindPalindromes_MultipleFound()
    {
        // AATT (4bp palindrome) and GAATTC (6bp palindrome)
        var results = RepeatFinder.FindPalindromes(
            new DnaSequence("GAATTC"), minLength: 4, maxLength: 6).ToList();

        // GAATTC(6bp) at 0, AATT(4bp) at 1
    ...
    {
        var results = RepeatFinder.FindPalindromes("GAATTC", minLength: 4, maxLength: 6).ToList();
        results.Should().NotBeEmpty();
    }
    ...
    {
        RepeatFinder.FindPalindromes("", minLength: 4, maxLength: 12).ToList().Should().BeEmpty();
    }

    [Test]
    public void FindPalindromes_NullSequence_Throws()
    {
        var act = () => RepeatFinder.FindPalindromes((DnaSequence)null!, minLength: 4, maxLength: 12).ToList();
        act.Should().Throw<ArgumentNullException>();
    }

    [Test]
    public void FindPalindromes_OddMinLength_Throws()
    {
        var act = () => RepeatFinder.FindPalindromes(new DnaSequence("GAATTC"), minLength: 5, maxLength: 12).ToList();
        act.Should().Throw<ArgumentOutOfRangeException>();
    }

    [Test]
    public void FindPalindromes_MinLengthTooSmall_Throws()
    {
        var act = () => RepeatFinder.FindPalindromes(new DnaSequence("GAATTC"), minLength: 2, maxLength: 12).ToList();
        act.Should().Throw<ArgumentOutOfRangeException>();
    }

    [Test]
    public void FindPalindromes_MaxLessThanMin_Throws()
    {
        var act = () => RepeatFinder.FindPalindromes(new DnaSequence("GAATTC"), minLength: 6, maxLength: 4).ToList();
        act.Should().Throw<ArgumentOutOfRangeException>();
    }
